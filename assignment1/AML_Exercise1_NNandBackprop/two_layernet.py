...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #scores = np.exp(Z3) / np.sum(np.exp(Z3)) not working properly for 2D array

        # CODE FOR (a)
        # scores = Z3.copy()
        # for i in range(len(Z3)):
        #     for j in range(len(Z3[i])):
        #         scores[i][j] = np.exp(Z3[i][j]) / np.sum(np.exp(Z3[i]))
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        probs_of_correct_class = [scores[i][y[i]] for i in range(len(scores))]
        # We take only probabilities of correct class

        loss = 1/N * np.sum(-np.log(probs_of_correct_class)) + reg*(np.linalg.norm(W1)**2 + np.linalg.norm(W2)**2)
        pass
